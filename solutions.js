...
try {
  let tests = [
    assert.strictEqual(outsideTempt.celsius, 10),
    assert.strictEqual(outsideTempt.getKelvinTemp(), 283.15),
    assert.strictEqual(outsideTempt.getFahrenheitTemp(), 50),
  ]
  console.log('4b)', `Passed ${tests.length} tests`)
} catch(e) {
  throw e
}
...
try {
  let tests = [
    assert.strictEqual(new Celsius(10).isBelowFreezing(), false),
    assert.strictEqual(new Celsius(0).isBelowFreezing(), false),
    assert.strictEqual(new Celsius(-10).isBelowFreezing(), true),
  ]
  console.log('4c)', `Passed ${tests.length} tests`)
} catch(e) {
  throw e
}
